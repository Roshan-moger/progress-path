...
const StudentResume = () => {
  ...
  const handleUpload = () => {
    setStage("analyzing");
    setTimeout(() => { completeStep("resume"); setStage("done"); }, 3000);
  };
        ...
        <AnimatePresence mode="wait">
          {stage === "upload" && (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              className={`border-2 border-dashed rounded-3xl p-16 text-center transition-colors ${dragActive ? "border-primary bg-accent/50" : "border-border bg-card"}`}
              ...
              <div className="w-20 h-20 rounded-2xl bg-accent mx-auto flex items-center justify-center mb-6"><Upload className="w-10 h-10 text-accent-foreground" /></div>
          ...
          {stage === "analyzing" && (
            <motion.div key="analyzing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }}
                className="w-20 h-20 rounded-2xl bg-primary/10 mx-auto flex items-center justify-center mb-6"><Sparkles className="w-10 h-10 text-primary" /></motion.div>
              ...
              <div className="mt-6 max-w-xs mx-auto"><div className="h-2 bg-muted rounded-full overflow-hidden">
                <motion.div className="h-full bg-primary rounded-full" initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 3 }} />
              </div></div>
            </motion.div>
          )}
          {stage === "done" && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border rounded-3xl p-10">
              ...
              <div className="bg-accent/50 rounded-2xl p-6 mb-6 border border-primary/10">
                <div className="flex items-center gap-3 mb-4"><FileText className="w-6 h-6 text-primary" /><span className="font-semibold text-foreground">Resume_2025.pdf</span></div>
                <div className="grid md:grid-cols-3 gap-4">
                  {[{ label: "Skills Detected", value: "React, Node.js, Python, MySQL, Git" }, { label: "Projects", value: "2 Projects found" }, { label: "Experience", value: "1 Internship at TechCorp" }].map((item) => (
