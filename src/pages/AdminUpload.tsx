          ...
          <div className="flex flex-wrap gap-2">
            {branches.map(b => (
              <button key={b} onClick={() => setSelectedBranch(b)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${selectedBranch === b ? "bg-primary text-primary-foreground shadow-md" : "bg-card border border-border text-muted-foreground hover:bg-accent"}`}>{b}</button>
            ))}
          </div>
        </div>
        <AnimatePresence mode="wait">
          {stage === "choose" && (
            <motion.div key="choose" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <motion.div whileHover={{ y: -4, scale: 1.01 }} onClick={() => setStage("excel")}
                className="bg-card border border-border rounded-2xl p-8 cursor-pointer hover:border-primary/30 transition-colors">
                <div className="w-16 h-16 rounded-2xl bg-accent flex items-center justify-center mb-5"><FileSpreadsheet className="w-8 h-8 text-accent-foreground" /></div>
              ...
              <motion.div whileHover={{ y: -4, scale: 1.01 }} onClick={() => setStage("manual")}
                className="bg-card border border-border rounded-2xl p-8 cursor-pointer hover:border-primary/30 transition-colors">
                <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mb-5"><UserPlus className="w-8 h-8 text-primary" /></div>
          ...
          {stage === "excel" && (
            <motion.div key="excel" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <motion.div className="bg-accent/50 border border-primary/10 rounded-2xl p-5 flex items-center justify-between mb-6">
                <div><p className="font-semibold text-foreground">Download Template</p><p className="text-sm text-muted-foreground">Columns: Name, USN, Email</p></div>
                <Button variant="outline" className="rounded-xl gap-2 border-primary/20 text-primary"><Download className="w-4 h-4" /> Template.xlsx</Button>
              </motion.div>
              <div className="border-2 border-dashed border-border rounded-3xl p-16 text-center bg-card">
                <div className="w-20 h-20 rounded-2xl bg-accent mx-auto flex items-center justify-center mb-6"><FileSpreadsheet className="w-10 h-10 text-accent-foreground" /></div>
                ...
                <Button onClick={handleUpload} className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl px-8">Browse Files</Button>
              </div>
            </motion.div>
          )}
          {stage === "manual" && (
            <motion.div key="manual" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div className="bg-card border border-border rounded-2xl p-6 mb-6">
                <h3 className="font-heading text-lg font-semibold mb-4 flex items-center gap-2"><Plus className="w-5 h-5 text-primary" /> Add Student to {selectedBranch}</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <input type="text" placeholder="USN (e.g. 1VY22CS001)" value={manualUSN} onChange={e => setManualUSN(e.target.value)}
                    className="h-11 px-4 rounded-xl bg-muted border border-border text-foreground placeholder:text-muted-foreground text-sm outline-none focus:ring-2 focus:ring-ring" />
                  <input type="email" placeholder="Email" value={manualEmail} onChange={e => setManualEmail(e.target.value)}
                    className="h-11 px-4 rounded-xl bg-muted border border-border text-foreground placeholder:text-muted-foreground text-sm outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <Button onClick={handleAddManual} className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl gap-2"><UserPlus className="w-4 h-4" /> Add Student</Button>
              </div>
              {addedStudents.length > 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card border border-border rounded-2xl overflow-hidden">
                  <div className="p-4 border-b border-border"><h3 className="font-semibold text-foreground">Added Students ({addedStudents.length})</h3></div>
          ...
          {stage === "processing" && (
            <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-card border border-border rounded-3xl p-16 text-center">
              <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }}
                className="w-20 h-20 rounded-2xl bg-primary/10 mx-auto flex items-center justify-center mb-6"><FileSpreadsheet className="w-10 h-10 text-primary" /></motion.div>
          ...
          {stage === "done" && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border rounded-3xl p-10 text-center">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }}
                className="w-20 h-20 rounded-full bg-success/10 mx-auto flex items-center justify-center mb-4"><CheckCircle className="w-12 h-12 text-success" /></motion.div>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <Link to={`${prefix}/admin/dashboard`}><Button className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl px-8">View Dashboard →</Button></Link>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
