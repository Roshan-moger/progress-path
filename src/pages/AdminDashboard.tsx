...
const AdminDashboard = () => {
  ...
  const filteredStudents = selectedBranch
    ? branchData[selectedBranch].students.filter(
          ...
          {!selectedBranch ? (
                      ...
                      <div className="p-6">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-heading text-lg font-bold text-foreground">{branch}</h3>
                              <p className="text-xs text-muted-foreground">{branch === "Diploma" ? "Diploma" : `B.Tech ${branch}`}</p>
                            </div>
                          </div>
            ...
            <motion.div key="table" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}>
              <div className="flex items-center gap-4 mb-6">
                <Button variant="ghost" onClick={() => { setSelectedBranch(null); setSearch(""); }} className="rounded-xl gap-2 text-muted-foreground">
                  <ArrowLeft className="w-4 h-4" /> Back
                </Button>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${branchData[selectedBranch].color} flex items-center justify-center`}>
                    <GraduationCap className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <h2 className="font-heading text-xl font-semibold">{selectedBranch}</h2>
                    <p className="text-xs text-muted-foreground">{branchData[selectedBranch].students.length} students</p>
                  </div>
                </div>
